...
* **CC12 – Location Cart:** Analog zu CC11, jedoch für den Standort des Kommissionierwagens. Zusätzlich wird hier der Status Transition between Areas (CL181) erfasst, wenn der Wagen eine Zonengrenze überquert.
...
### 2. Keine Mehrdeutigkeit innerhalb eines Frames (Priorisierung)

Innerhalb eines Frames darf eine Klasse niemals mehrere gleichzeitig konkurrierende Zustände besitzen.

**Exklusivität:** Ein Subjekt kann nicht gleichzeitig Walking und Sitting sein (CC01).

**Priorisierung bei CC01:** Falls physisch mehrere Aktivitäten gleichzeitig auftreten (z. B. Gehen und Scannen), dominiert die semantisch höherwertige Tätigkeit gemäß der definierten Hierarchie (z. B. Scanning > Walking).
...
Diese prozedurale Einordnung bildet die Brücke zu arbeitswissenschaftlichen Zeitarten (z. B. $t_{MH}$ für Pick Time, $t_{MN}$ für Travel Time) gemäß REFA.
...
* die Interpretation von Szenariotypen (Single-Order vs. Multi-Order, Szenarien mit Intentional Errors vs. Perfect Runs).
...
ableitbar sind, werden nicht ergänzt. Es erfolgt keine semantische Inferenzen über nicht belegte Zusammenhänge.
...
* Konsistenzregeln und Priorisierungen (CC01)
* Interpretation der Annotation

**Nicht in dieser Datei:**
* Label-Definitionen → Siehe [labels_207.md](../core/labels_207.md)
* Chunking-Logik → Siehe [chunking.md](chunking.md)
* BPMN-Prozesslogik → Siehe [dataset_core.md](dataset_core.md#16-offizielle-prozesslogik-bpmn)
* Frame-Struktur → Siehe [data_structure.md](data_structure.md#56-frame-struktur)
* Szenario-Definitionen → Siehe [ground_truth_central.md](../core/ground_truth_central.md)

---

**Skill-Version:** 1.2 (erweitert mit Semantik-Details und REFA-Bezug)
