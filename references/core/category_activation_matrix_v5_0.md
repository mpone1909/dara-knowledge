...
Zentrale Referenzdatei für die **Min/Max-Aktivierungslogik aller 12 Klassenkategorien (CC01-CC12)** des DaRa Datasets. Enthält:
- Aktivierungsregeln (Kardinalität: Min/Max pro Frame)
- Erlaubte Kombinationen (Multi-Label)
- Explizite Widerspruchsmatrix (Verbotene Kombinationen)
- Abhängigkeitsregeln zwischen Kategorien
- Prioritätsregeln bei Gleichzeitigkeit
- Validierungs-Pseudocode

**Verwandte Dateien:**
- `core_labels_207_v5.0.md` — Struktur und Definition aller 207 Labels
- `core_ground_truth_central_v5.0.md` — Szenario-Definitionen (S1-S8)

---

## 1. SCHNELLÜBERSICHT: Min/Max-Matrix

| Kategorie | Min. aktiv | Max. aktiv | Typ | Beschreibung | Kardinalität-Regel |
|-----------|-----------|-----------|-----|--------------|-------------------|
...
| **CC06** | 1–2 | 1–2 | Variabel | Order | Min/Max hängt von Szenario ab (S1-S3: 1, S4-S6: 1, S7-S8: 2, Other: 1) |
...
| **CC09** | 1 | 1 | Single | Mid-Level Process | Immer genau 1 Label (Master → CC10, abhängig von CC08) |
| **CC10** | 1 | 1 | Single | Low-Level Process | Immer genau 1 Label (abhängig von CC09) |
| **CC11** | 1 | 3 | Hierarchisch | Location – Human | 1 = einfacher Bereich; 3 = Regalgang (Aisle + Nummer + Position) |
| **CC12** | 1 | 4 | Hierarchisch | Location – Cart | 1–3 = wie CC11; +1 optional CL181 (Transition) |

---

## 2. SINGLE-LABEL-KATEGORIEN (6 Kategorien: CC01, CC02, CC07–CC10)

Diese Kategorien dürfen **nur ein Label pro Frame haben**. Alle Labels sind untereinander exklusiv.
...
- CL004: Reaching
- CL005: Scanning
- CL006: [weitere Aktivitäten...]
- CL015: Standing

**Rolle:** Master für alle motorischen Kategorien (CC02–CC05). Definiert Hauptaktivität.
...
- ❌ Walking (CL011) + Standing (CL015) **gleichzeitig** → Exklusiv
- ❌ Sitting (CL013) + Scanning (CL005) → Exklusiv (auch wenn Scanning während Sitzen möglich)

**Prioritätsregel bei Mehrdeutigkeit:**
Falls mehrere Aktivitäten gleichzeitig wahrnehmbar sind, gilt diese Prioritätsordnung:
```
Sync > Confirming > Scanning > Cart (Pushing) > Handling > Walking/Standing
```

**Beispiel:** Person scannt einen Artikel während sie geht:
- Kandidaten: Scanning (CL005), Walking (CL011)
- Priorität: Scanning hat Vorrang → CL005 wird gewählt
...
- CL018: Squat (Hocke — beinhaltet auch kleine Schritte)
- [weitere Labels...]
...
**Widerspruchs-Beispiele:**
- ❌ Gait Cycle (CL016) + Standing Still (CL017) → Exklusiv
- ❌ Zwei Bewegungstypen gleichzeitig
...
**Widerspruchs-Beispiele:**
- ❌ List + Pen (CL105) + Scanner (CL106) → Exklusiv (nur ein IT-System zur selben Zeit)
...
**Rolle:** Master für CC09. Definiert Prozesstyp (Kommissionierung vs. Einlagerung).

**Abhängigkeit:** CL111 (Storage) aktiviert nur Storage-spezifische CC09-Labels (z.B. CL117 Unpacking, CL119 Storing Travel).
...
- CL118: Packing
- CL121: Finalizing Order (auch in Storage möglich)

**Storage-Prozesse (wenn CC08 = CL111):**
- CL117: Unpacking
- CL119: Storing – Travel Time
- CL120: Storing – Store Time
- CL121: Finalizing Order (auch in Retrieval möglich)

**Rolle:** Master für CC10. Hauptdiskriminator für REFA-Zeitarten.

**Abhängigkeitsregeln:**
- CL117 (Unpacking) nur bei CC08 = Storage (CL111)
- CL121 (Finalizing Order) gültig bei BEIDEN Retrieval und Storage
- Abhängig von CC09 → definiert mögliche CC10-Labels
...
**Beispiel-Labels:**
- CL124: Walking
- CL135: Error – Report to Supervisor (Error-Marker)
- [weitere atomare Prozessschritte...]

**Rolle:** Detaillierung von CC09. Atomare Prozessschritte.

**Abhängigkeitsregeln:**
- Wenn CC09 = CL118 (Packing), nur Packing-spezifische CC10-Labels gültig
- Wenn CC09 = CL120 (Storing – Store Time), nur Storing-spezifische CC10-Labels gültig
...
**Widerspruchs-Beispiele:**
```
❌ CC03 = [CL027]                   Rotation ohne Biegung (UNGÜLTIG)
❌ CC03 = [CL024, CL026]            Zwei verschiedene Beugungsgrade (exklusiv)
❌ CC03 = [CL024, CL025, CL026]     Drei Beugungsgrade (max 1 erlaubt)
```
...
3. **Object (genau 1):**
   ...
   - CL044: Tool
   - CL045–CL050: [weitere Objekt-Labels...]
   - CL051: Object Unknown

4. **Tool (genau 1):**
   - CL052: Portable Data Terminal
   - CL053: Glove Scanner
   - CL054–CL063: [weitere Tool-Labels...]
   - CL064: Another Tool
...
**Erlaubte Kombinationen:**
```
✓ CC04 = [CL031, CL034, CL043, CL056]
  (Centered + Grasping + Small Item + Pen)
  
✓ CC04 = [CL031, CL037, CL040, CL064]
  (Centered + No Movement + No Object + Another Tool)
  Hinweis: Trotz Inaktivität (CL037) müssen alle 4 Gruppen belegt sein!
```

**Widerspruchs-Beispiele:**
```
❌ CC04 = [CL031, CL034, CL035, CL043, CL056]
   (5 Labels statt 4)
   
❌ CC04 = [CL031, CL034, CL035, CL043, CL056]
   (zwei Movement-Labels: CL034 + CL035 — exklusiv)
   
❌ CC04 = [CL031, CL034, CL043]
   (nur 3 Labels — Tool-Gruppe fehlt)
```
...
**Labels:** CL065–CL099 (35 Labels, Struktur identisch zu CC04)

**Untergruppen:**
- Position: CL065–CL068 (analog zu CC04)
- Movement: CL069–CL074 (analog zu CC04)
- Object: CL075–CL086 (analog zu CC04)
- Tool: CL087–CL099 (analog zu CC04)

**Validierungsregel:** Identisch zu CC04, aber mit Range CL065–CL099.

---

## 4. SZENARIO-ABHÄNGIGES ORDER-LABEL (CC06)

**Kardinalität:** Min/Max variabel (abhängig von Szenario)
...
**Validierungsregel (Szenario-abhängig):**
```python
def validate_cc06(active_orders, scenario):
    # S1–S3: Single-Order mit IT-Diskriminator
    ...
    # S4–S6: Szenario-spezifische Single-Order
    elif scenario == 'S4':
    ...
    # S7–S8: Multi-Order
        ...
        assert set(active_orders) == {'CL100', 'CL101'}, \
            f"{scenario}: Orders müssen CL100 + CL101 sein"
...
**Widerspruchs-Beispiele:**
```
❌ S4 + [CL101]                     Szenario verlangt CL100
❌ S1 + [CL100, CL101]              S1 erlaubt nur 1 Order (nicht Multi)
❌ S7 + [CL100, CL103]              S7 verlangt CL100 + CL101 (nicht CL103)
❌ Any Scenario + [CL103, CL104]    No Order und Order Unknown sind exklusiv
```
...
```
✓ CC11 = [CL155]
  (Office — einfacher Bereich)
  
✓ CC11 = [CL163, CL174, CL177]
  (Aisle Path + Aisle 3 + Front — Regalgang Gasse 3, vorne)
  
✓ CC11 = [CL158]
  (Cardboard Box Area — einfacher Bereich)
```

**Widerspruchs-Beispiele:**
```
❌ CC11 = [CL155, CL156]
   (zwei Hauptbereiche — exklusiv)
   
❌ CC11 = [CL163, CL174]
   (Regalgang unvollständig — Position fehlt)
   
❌ CC11 = [CL163, CL174, CL177, CL178]
   (4 Labels — max. 3 erlaubt)
   
❌ CC11 = [CL155, CL163, CL174, CL177]
   (Mixed: einfacher Bereich + Regalgang — nicht kombinierbar)
```
...
**Struktur:**
- CC11-Logik (CL155–CL180) + optionales CL181 (Transition)
...
def validate_cc12_location_cart(location_labels):
    ...
    if aisle_marker in base_labels:
        ...
        # Außerhalb Regalgang: 1 Base-Label
...
✓ CC12 = [CL163, CL173, CL177, CL181]
  (Wagen überquert Zonengrenze in Regalgang 2)
  
✓ CC12 = [CL156, CL181]
  (Wagen überquert Zone von Base)
```
...
❌ CC12 = [CL181]
   (nur Transition — mindestens 1 Bereich erforderlich)
```
...
| **CC01** | Scanning (CL005) + Walking (CL011) bei Priorität | Scanning hat Vorrang (Prioritätsregel) |
...
| **CC04** | 2× Labels aus Position-Gruppe (z.B. CL030 + CL031) | Genau 1 pro Gruppe erforderlich |
| **CC04** | Grasping (CL034) + Holding (CL035) | 2 Movement-Labels: exklusiv |
| **CC04** | 3 Labels statt 4 (z.B. Position + Movement + Object, kein Tool) | Alle 4 Gruppen müssen belegt sein |
| **CC05** | Wie CC04, aber für Right Hand | Identische Logik |
...
| **CC11** | Aisle Path (CL163) + Gassennummer (CL172) ohne Position (CL177/CL178) | Regalgang-Struktur unvollständig |
...
**CC08 (High-Level) → CC09 (Mid-Level) → CC10 (Low-Level)**

Diese Kategorien bilden eine strikte Hierarchie:
- **CC08** definiert den Grobprozess (Retrieval vs. Storage)
- **CC09** ist abhängig von CC08: nur gültige Prozesslabels je nach Grobprozess
- **CC10** ist abhängig von CC09: nur gültige Schritt-Labels je nach Prozessphase
...
CC08 = CL110 (Retrieval)
  ...
  ↓ (wenn CC09 = CL115)
  Gültige CC10-Labels: [Picking-Travel-spezifische Labels]
  
  ↓ (wenn CC09 = CL121)
  Gültige CC10-Labels: [Finalizing-spezifische Labels]
...
CC08 = CL111 (Storage)
  ...
  ↓ (wenn CC09 = CL120)
  Gültige CC10-Labels: [Storing-Store-spezifische Labels]
```

**Spezialfall: CL121 (Finalizing Order)**
- Gültig in BEIDEN Retrieval (CC08 = CL110) UND Storage (CC08 = CL111)
- Dies ist korrekt und nicht redundant; der Finalisierungsprozess tritt in beiden Szenarien auf

### CC01 als Master für motorische Kategorien

**CC01 (Main Activity) → CC02, CC03, CC04, CC05**

- **CC01** definiert die Hauptaktivität
- **CC02–CC05** bieten Detaillierung der motorischen Aspekte
- Diese sind relativ unabhängig von CC01, können aber kontextabhängig sein

**Beispiel:**
```
CC01 = CL004 (Reaching)
  → CC04/CC05 sollten "Grasping" oder "Reaching" enthalten (konsistent)

CC01 = CL011 (Walking)
  → CC02 sollte "Gait Cycle" enthalten (konsistent)
```

### CC06 ↔ CC07 als Szenario-Diskriminatoren

**CC06 (Order) ↔ CC07 (Information Technology) ↔ CC08 (High-Level Process)**

Diese drei Kategorien sind bei S1–S3 eng verknüpft:
- **CC07** (IT) kann zur Szenario-Erkennung genutzt werden
- **CC06** (Order) folgt oft dem durch CC07 erkannten Szenario
- **CC08** (High-Level) ist szenario-unabhängig, aber kann konsistent mit CC06/CC07 sein

---

## 8. KARDINALITÄTS-ZUSAMMENFASSUNG
...
| **Multi-Label (Optional)** | CC03, CC06 | 1–2 | 1–2 | Variabel je Szenario/Kontext |
| **Multi-Label (Zwingend)** | CC04, CC05 | 4 | 4 | 4 Gruppen à 1 Label |
...
Für S1–S8 gelten unterschiedliche Kardinalitäten und Abhängigkeiten.

**Exemplarische Regel für S4 (Storage mit Order 2904):**
```
Frame in Szenario S4:
  CC01: Exactly 1 (Main Activity)
  CC02: Exactly 1 (Leg Movement)
  CC03: 1–2 (Torso optional)
  CC04–CC05: 4 each (Hands)
  CC06: CL100 (Order 2904 — zwingend!)
  CC07: CL108 oder CL109 (No IT / Unknown — S4 ist Storage)
  CC08: CL111 (Storage)
...
**Verwandte Dateien:**
- `core_labels_207_v5.0.md` — Vollständige Definition aller 207 Labels
- `core_ground_truth_central_v5.0.md` — Szenario-Definitionen und 5-Schritt Decision-Tree
- `assets_query_patterns_v5.0.md` — Such-Muster pro Label und Kategorie

**Änderungshistorie:**
- v5.0 (2026-02-04): Neuerstelle von v2.6.1 mit vollständiger Kombinationslogik, Widerspruchs-Matrix, Abhängigkeitsregeln und Prioritätsregeln
